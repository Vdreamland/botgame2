...
| Group | `type` values |
|-------|---------------|
| Movement / vision | `agent_moved`, `monster_moved` |
...
`view.connectedRegions` contains adjacent region IDs as **string** values.
To get the full region object, look it up in `view.visibleRegions`.
...
# 12.1 pendingDeathzones Entry Shape

`view.pendingDeathzones` is an array of `{ id, name }` entries (not region IDs alone).
