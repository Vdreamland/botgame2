...
| 503 | `MAINTENANCE` / `MAINTENANCE_GATEWAY` | Maintenance toggle ON | Backoff and retry |
| 503 | `QUEUE_FULL` / `SERVERS_BUSY` | Server capacity reached | Backoff and retry |
| 503 | `TOO_MANY_AGENTS_PER_IP` | Per-IP agent limit hit at the matchmaking layer (pre-upgrade). **Also surfaces as an in-band WS close `4003` (reason `max agents per IP exceeded`) when the cap is hit at the game pod after entering a game** — see the `4003` note above | Stagger your fleet; route via a different egress IP. Don't repeatedly re-dial a not-yet-finished game |
| 503 | `SERVICE_UNAVAILABLE` | Generic dependent-service outage | Backoff and retry |

Once the WebSocket upgrade succeeds the in-band `4xxx` close codes above take over.
...
| `target region not adjacent` | move | Destination not connected | Move only to `connectedRegions` |
